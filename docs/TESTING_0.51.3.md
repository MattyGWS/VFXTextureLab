# Testing VFX Texture Lab 0.51.3

## Setup requirement

After copying this archive over an older working folder, run `setup.sh` on Linux or `setup.bat` on Windows once. This installs the new compiled `fast-simplification` dependency into the existing `.venv`.

Focus a Geometry Decimate node after setup. **Simplification Diagnostics → Backend** should report **Native QEM (fast-simplification)**. A Python fallback warning means setup has not installed the new dependency.
