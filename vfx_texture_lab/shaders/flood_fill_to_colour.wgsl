...
fn sample_bilinear(tex: texture_2d<f32>, uv_value: vec2<f32>) -> vec4<f32> {
    let dims_u = textureDimensions(tex); let dims = vec2<f32>(dims_u);
    ...
    return mix(mix(textureLoad(tex,p00,0), textureLoad(tex,p10,0), f.x), mix(textureLoad(tex,p01,0), textureLoad(tex,p11,0), f.x), f.y);
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (!is_active(data)) { textureStore(output_tex, coord, vec4<f32>(0.0)); return; }
    var base = params.p1;
    if (params.p2.w >= 0.5) { base = sample_bilinear(colour_tex, data.rg); }
    let seed = u32(max(params.p2.z, 0.0));
    let random = vec3<f32>(flood_hash(data, seed, 3u), flood_hash(data, seed, 4u), flood_hash(data, seed, 5u)) * 2.0 - vec3<f32>(1.0);
    let rgb = clamp(base.rgb + vec3<f32>(params.p2.x) + random * params.p2.y, vec3<f32>(0.0), vec3<f32>(1.0));
    textureStore(output_tex, coord, vec4<f32>(rgb, base.a));
}
