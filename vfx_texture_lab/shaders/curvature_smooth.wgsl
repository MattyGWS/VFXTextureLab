...
fn normal_at(coord: vec2<i32>) -> vec3<f32> {
    ...
    if (params.p1.x >= 0.5) { value.y = -value.y; }
    let magnitude = length(value);
    if (magnitude <= 0.0000001) { return vec3<f32>(0.0, 0.0, 1.0); }
    return value / magnitude;
}

fn divergence_at(p: vec2<i32>, radius: i32) -> f32 {
    ...
    return ((right - left) + (down - up)) * 0.25;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let r1 = max(i32(round(params.p1.z)), 1);
    let r2 = max(i32(round(params.p1.w)), 1);
    let r4 = max(i32(round(params.p2.x)), 1);
    let curvature = 2.0 * (
        divergence_at(p, r1) * 0.50
        + divergence_at(p, r2) * 0.30
        + divergence_at(p, r4) * 0.20
    );
    let output_index = u32(round(params.p1.y));
    var value = clamp(0.5 + 0.5 * curvature, 0.0, 1.0);
    if (output_index == 1u) {
        value = clamp(curvature, 0.0, 1.0);
    } else if (output_index == 2u) {
        value = clamp(-curvature, 0.0, 1.0);
    } else if (abs(curvature) <= 0.0000000001) {
        value = 0.5;
    }
