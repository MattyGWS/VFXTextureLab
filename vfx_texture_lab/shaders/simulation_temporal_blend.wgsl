...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let current = textureLoad(current_tex, c, 0);
    let history = textureLoad(history_tex, c, 0);
    let persistence = clamp(params.p1.x, 0.0, 0.9999);
    let rgb = mix(current.rgb, history.rgb, persistence);
    textureStore(output_tex, c, vec4<f32>(rgb, current.a));
}
