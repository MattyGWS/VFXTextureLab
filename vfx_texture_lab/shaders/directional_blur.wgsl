...
fn sample_bilinear(position: vec2<f32>, dimensions: vec2<i32>) -> vec4<f32> {
    ...
    let base = vec2<i32>(floor(wrapped));
    let fraction = fract(wrapped);
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let distance = max(params.p1.x, 0.0);
    let radians = params.p1.y * 0.017453292519943295;
    let samples = clamp(i32(params.p1.z + 0.5), 2, 128);
    let direction = vec2<f32>(cos(radians), sin(radians)) * distance;
    let pixel = vec2<f32>(gid.xy);
    let dimensions = vec2<i32>(width, height);
    var total = vec4<f32>(0.0);
    for (var index: i32 = 0; index < 128; index = index + 1) {
        if (index < samples) {
            let t = (f32(index) / f32(samples - 1) - 0.5) * 2.0;
            total = total + sample_bilinear(pixel + direction * t, dimensions);
        }
    }
