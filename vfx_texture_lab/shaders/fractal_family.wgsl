...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let packed_mode = i32(round(params.p3.w));
    let mode = packed_mode & 7;
    let invert = (packed_mode & 8) != 0;
    ...
    for (var octave: u32 = 0u; octave < 10u; octave = octave + 1u) {
        ...
        let base = noise_periodic_gradient3(
            uv, cells, seed + octave * 1013u, loop_data.x, u32(loop_data.y)
        );
        ...
        if (mode == 1) {
            ...
            sample = pow(clamp(abs(signed_value), 0.0, 1.0), 0.72);
        }
    ...
    if (mode == 3) { value = clamp(value * 1.15, 0.0, 1.0); }
