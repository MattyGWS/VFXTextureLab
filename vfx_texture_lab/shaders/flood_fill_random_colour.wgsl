...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let data = textureLoad(flood_tex, coord, 0);
    if (!is_active(data)) { textureStore(output_tex, coord, vec4<f32>(0.0)); return; }
    let seed = u32(max(params.p1.x, 0.0));
    let colour = vec3<f32>(flood_hash(data, seed, 0u), flood_hash(data, seed, 1u), flood_hash(data, seed, 2u));
    textureStore(output_tex, coord, vec4<f32>(colour, 1.0));
}
