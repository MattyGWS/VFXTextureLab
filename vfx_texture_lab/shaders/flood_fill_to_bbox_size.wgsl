...
fn main(@builtin(global_invocation_id) gid:vec3<u32>){
 ...
 let coord=vec2<i32>(gid.xy);let data=textureLoad(flood_tex,coord,0);var value=0.0;
 if(is_active(data)){let size=flood_size(data);let mode=i32(params.p1.x+0.5);
  if(mode==1){value=min(size.x,size.y);}else if(mode==2){value=size.x;}else if(mode==3){value=size.y;}else if(mode==4){value=size.x*size.y;}else{value=max(size.x,size.y);}}
